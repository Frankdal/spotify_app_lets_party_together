...
app.route('/playlist')
    .post(async (req, res) =>  {
        ...
        if (result.status != 200 && result.status != 201) res.status(result.status).send("Couldn't post the song on the Playlist " + result.statusMessage); 
        
        let data = await spotify.getPlaylistItems(playlistId, token); 
        // console.log(data); 
        if(data.status) res.status(data.status).send("Couldn't get songs from playlist: " + data.statusMessage); 
        ...
        if(features.status) res.status(features.status).send("Couldn't get the features: " + features.statusMessage); 
        ...
        for(i = 0; i<features.audio_features.length; i++) {
            clusters[clusteringResult.idxs[i]].push(features.audio_features[i].uri); 
        }
        ...
        let replacementResult = await spotify.replacePlaylistSongs(playlistId, trackUris, token);
        res.status(replacementResult.status).send(replacementResult); 
        }
        else {
            let rearrangeResult = await listManager.rearrangePlaylist(playlistId, token, data);
